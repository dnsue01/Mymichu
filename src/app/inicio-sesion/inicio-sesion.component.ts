...
export class InicioSesionComponent {
  ...
  comprobacion() {
    this.ConexionPhpService.comprobarUsuarioInicio(this.usuario).subscribe((datos: any) => {
      if (datos) {
        this.inicioSesion()
  ...
  inicioSesion() {
    this.ConexionPhpService.iniciarSesion(this.usuario).subscribe((datos: any) => {
      if (datos) {
  ...
  comprobarAdmin() {
    this.ConexionPhpService.comprobarAdmin(this.idUsuario).subscribe((datos: any) => {
      //devuelve true si es administrador false si es un usuario normal
      if(datos["mensaje"]){
       this.entrarAdmin(this.idUsuario)
        
      }else{
        this.entrarUsuario(this.idUsuario)
      }
